...
- 🎤 **语音输入**：浏览器语音识别，说出需求即可生成（Chrome / Edge；需 HTTPS 安全上下文，见下文「HTTPS 与语音输入」）
...
语音输入依赖浏览器麦克风权限，而麦克风权限要求「安全上下文」（HTTPS 或 localhost）。

- **本地开发**：`http://localhost:5173` 本身即安全上下文，语音可直接使用。
- **线上部署**：`http://IP:8082` 非安全上下文，语音不可用；应用已内置自签名 HTTPS 监听（默认 8443 端口），访问 `https://<IP>:8443`（浏览器会提示证书不安全，点「高级 → 继续访问」一次即可）即可使用语音。

自签名证书由 `server/index.js` 读取 `server/key.pem` + `server/cert.pem` 自动启用，可用 openssl 重新生成；端口/路径可通过 `HTTPS_PORT` / `HTTPS_KEY` / `HTTPS_CERT` 环境变量覆盖。
